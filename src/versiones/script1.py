...
def convert_markdown_to_blogger(html_file, markdown_file):
    ...
    # Expresión regular para encontrar bloques de código en Markdown antes de la conversión
    code_block_pattern = re.compile(r'```(.*?)```', re.DOTALL)

    # Función para reemplazar cada bloque de código con el formato deseado
    def replace_code_block(match):
        code = match.group(1).strip()  # código dentro del bloque
        # Escapar entidades HTML dentro del código para mostrar correctamente
        code = re.sub(r'&', '&amp;', code)
        code = re.sub(r'<', '&lt;', code)
        code = re.sub(r'>', '&gt;', code)
        code = re.sub(r'"', '&quot;', code)
        code = re.sub(r"'", '&#39;', code)
        # Crear la caja de código personalizada
    ...
    # Reemplaza los bloques de código en el contenido Markdown por las cajas de código personalizadas antes de la conversión
    markdown_content = re.sub(code_block_pattern, replace_code_block, markdown_content)

    # Convierte el contenido Markdown (incluyendo los bloques de código ya reemplazados) a HTML
    ...
    # Agregar párrafos vacíos antes y después de las listas
    for tag in soup.find_all(['ol', 'ul']):
